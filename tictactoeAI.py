...
class Board: 
    ...
    def __init__(self):
        ...
        self.marked_sqrs = 0 # Number of marked squares

    # 
    # @return 0 if there is no win next move
    # @return 1 if player 1 has a win next move
    # @return 2 if player 2 has a win next move
    #
    def terminal_state(self, show=False):
        # Vertical Wins
...
class AI:
    ...
    def eval(self, main_board):
        if self.level == 0:
            ...
            # Run minimax algo
            eval, move = self.minimax(main_board, False) # AI wants to minimize so use false
...
class Game:
    ...
    def is_over(self):
        return self.board.terminal_state(show=True) != 0 or self.board.is_full()
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # G Key = change gamemode (pvp or ai)
